...
model<- function(df){ 
  ...
  df <- data.frame(df, v, v_CS_US, delta_v_CS_US,  v_US_CS,  delta_v_US_CS, cs_oriented, us_oriented)

  }

#Transform model to long
to.long<- function(df){
  df_long<- gather(df, ass_indicator, value_ass, - trial, -alpha, -beta,  -trial_type, -cs_oriented, -us_oriented)
  df_long<- gather(df_long, response_type, value_response, - trial, -alpha, -beta, -trial_type,  -ass_indicator, -value_ass)
}
